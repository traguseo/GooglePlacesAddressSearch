/*
   ...
   Company:        Salesforce
   Description:    src/aura/AddressValidation/GooglePlacesAddressSearchHelper.js
...
({
    ...
    setValidation: function (cmp) {
        let labels = {
            SEARCH_AND_ALL_ADDRESS_FIELDS_REQUIRED: $A.get("$Label.c.GPAS_Search_and_All_Address_Fields_Required"),
            SEARCH_AND_ADDRESS_FIELD_REQUIRED: $A.get("$Label.c.GPAS_Search_and_Address_Field_Required"),
            FILLED_ADDRESS_FIELDS_REQUIRED: $A.get("$Label.c.GPAS_Filled_Address_Fields_Required")
        };
        ...
        cmp.set("v.validate", function () {
            ...
            if (fieldsRequired && isRequired) {
                ...
                else return { isValid: false, errorMessage: labels.SEARCH_AND_ADDRESS_FIELD_REQUIRED };
            } else if (showAddressFields && fieldsRequired) {
                if (!addressFieldEmpty) return { isValid: true };
                else return { isValid: false, errorMessage: labels.FILLED_ADDRESS_FIELDS_REQUIRED };
            }
        });
    },
    initialiseMapData: function (cmp, helper) {
        ...
        let labels = {
            SELECTED_ADDRESS: $A.get("$Label.c.GPAS_Selected_Address"),
            DEFAULT_ADDRESS: $A.get("$Label.c.GPAS_Default_Address"),
            CURRENT_LOCATION: $A.get("$Label.c.GPAS_Current_Location"),
            ENABLE_LOCATION_TRACKING: $A.get("$Label.c.GPAS_Enable_Location_Tracking"),
            POSITION_UNAVAILABLE: $A.get("$Label.c.GPAS_Position_Unavailable"),
            REQUEST_TIMED_OUT: $A.get("$Label.c.GPAS_Request_Timed_Out"),
            UNKNOWN_ERROR: $A.get("$Label.c.GPAS_Unknown_Error")
        };
    ...
    startSearch: function (cmp) {
        let labels = {
            CONTACT_ADMIN: $A.get("$Label.c.GPAS_Please_contact_your_admin"),
            API_UNKNOWN_ERROR: $A.get("$Label.c.GPAS_API_Unknown_Error")
        };
                ...
                if (placeDetails.status === "OK") {
                    ...
                    for (i = 0; i < place.address_components.length; i++) {
                        ...
                        for (var j = 0; j < addressTypes.length; j++) {
                            if (componentForm.hasOwnProperty(addressTypes[j])) {
                    ...
                    // If the place is an establishment, prepend the name to the street address
                    if (place.types.includes("establishment")) {
                        fullStreetAddress = place.name + "\n";
                    }

                    // Build the full street address from the components that exist / aren't null
                    fullStreetAddress += [
                        cmp.get("v.street_number"),
                        cmp.get("v.route"),
                        cmp.get("v.premise"),
                        cmp.get("v.subpremise")
                    ]
    ...
    clearAddressFields: function (cmp) {
        ...
        cmp.set("v.subpremise", null);
    },
    checkValidFilter: function (cmp) {
        const labels = {
            COUNTRY_FILTER_LIMIIT: $A.get("$Label.c.GPAS_Country_Filter_Limit"),
            COUNTRY_FILTER_FORMAT: $A.get("$Label.c.GPAS_Country_Filter_Format")
        };
